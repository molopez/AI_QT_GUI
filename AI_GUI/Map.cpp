...
{
    return sqrt(pow((double)(a.getXCoordinate() - b.getXCoordinate()),2) + pow((double)(a.getYCoordinate() - b.getYCoordinate()),2));
}
...
list<string> Map::getCities()
{
    for (vector<City>::iterator it = cities.begin(); it != cities.end(); ++it)
    {
        allCities.push_back(it->getCityName());
    }
    return allCities;
}
